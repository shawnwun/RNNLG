    ...
    for p in percent:
        for g in gtypes:
            for s in seed:
                ...
                oldmodel = 'model/batch/'+identify+'.model'
                newmodel = 'model/batch/'+identify+'.model-adpML-'+nwset.replace('/','@')+'-'+p
                                    
                # writing config file
                text = template(newmodel,mode,obj,batch,g,hidden,
                        dom,nwset,p,s,vecfile=vecfile,decode=dec,
                        lr='0.05')
                config ='config/batch/'+identify+'.cfg-adpML-'+nwset.replace('/','@')+'-'+p
                ...
                # script and log file
                scp = 'scp/batch/'+identify+'.sh-adpML-'+nwset.replace('/','@')+'-'+p
